...
import srcs.rmi.service.SystemDeployer;
...
public class ServiceSansEtatTest extends SystemDeployer {
	...
	public void testServiceSansEtat() throws RemoteException, NotBoundException {
		...
		FunctionService<Operandes,Double> div1 = (FunctionService<Operandes, Double>) s1.deployNewService(nameservice, DivisionService.class );
		Operandes operandes =new Operandes(5.0, 2.0);
		
		
		assertEquals(2.5, (Double)div1.invoke(operandes),0.01); 

		FunctionService div2 = div1.migrateTo(s2);
		assertEquals(2.5, (Double)div1.invoke(operandes),0.01); 
		assertEquals(2.5, (Double)div2.invoke(operandes),0.01); 
		
		assertThrows(RemoteException.class, ()->div1.migrateTo(s2));
		...
		assertThrows(NoSuchObjectException.class,()->div1.getName());
		
		FunctionService div3 = div2.migrateTo(s1);
		assertTrue(s1.getServices().contains(nameservice));
		assertTrue(s2.getServices().contains(nameservice));
		...
		assertThrows(ServerException.class,()->div2.invoke(operandes));
